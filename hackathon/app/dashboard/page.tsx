...
import { DemoWorkflowData, emptyWorkflowData, getWorkflowCounts, loadWorkflowData } from '@/lib/demo-workflow'
...
export default function DashboardPage() {
  ...
  const [workflowData, setWorkflowData] = useState<DemoWorkflowData>(emptyWorkflowData)
  ...
  const workflowCounts = getWorkflowCounts(workflowData)
  const activeVendors = DEMO_VENDORS.filter(v => v.status === 'active').length + workflowData.vendors.length
  const openRFQs = DEMO_RFQS.filter(r => r.status === 'open').length + workflowData.rfqs.filter(r => r.status === 'open').length
  const pendingApprovals = DEMO_QUOTATIONS.filter(q => q.status === 'submitted').length + workflowData.quotations.length
  const monthlySpend = MONTHLY_SPEND_DATA[MONTHLY_SPEND_DATA.length - 1].actual

  const QUICK_ACTIONS = [
    { label: 'Input Center', icon: <Plus size={16} />, href: '/dashboard/input', color: '#6366F1', bg: '#EEF2FF' },
    ...
    <div style={{ maxWidth: '1400px', animation: 'fadeIn 0.4s ease' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#64748B', fontSize: '14px' }}>
          Here&apos;s what&apos;s happening with your procurement today · {workflowCounts.rfqs} total RFQs including input records
        </p>
      </div>
